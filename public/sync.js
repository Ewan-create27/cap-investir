...
const cloud={mode:"cap",ready:false,guest:false,user:null,spaces:[],space:null,revision:0,pending:null,running:null,error:'',conflict:null,generation:0,busy:false,createId:null};
...
// Visitor data lives only in this page's memory, never in account storage.
function startGuest(){
 cloud.guest=true;cloud.user=null;cloud.spaces=[];cloud.space={id:'guest',name:'Visiteur'};
 cloud.ready=true;cloud.revision=0;cloud.pending=null;cloud.error='';cloud.conflict=null;cloud.generation++;
 state=normalizedState(blankState());filter='Tout';render();
}
...
function applySpace(row){cloud.guest=false;state=normalizedState(row.data);loadActiveGoal();cloud.space={id:row.id,name:row.name};cloud.revision=row.revision;cloud.ready=true;cloud.error='';cloud.conflict=null;cloud.generation++;rememberSpace();render();}
function accountChrome(){
 ...
 const status=document.querySelector('#sync-status');if(status){status.textContent=cloud.guest?'Visiteur · non sauvegardé':cloud.conflict?'Conflit à résoudre':cloud.error?'Non synchronisé':cloud.pending||cloud.running?'Enregistrement…':cloud.ready?'Sauvegardé en ligne':'Connexion…';status.dataset.warning=String(Boolean(cloud.error||cloud.conflict));}
...
async function startCloud(){
 ...
 }catch(error){if(error.status===401&&!cloud.user){startGuest();}else{cloud.error=error.message;render();}}
 finally{cloud.busy=false;if(!cloud.ready)render();else accountChrome();}
}
function queueCloudSave(){if(!cloud.ready||cloud.guest)return;cloud.pending=JSON.stringify(state);cloud.generation++;accountChrome();void flushCloud();}
async function flushCloud(){
 if(cloud.guest)return true;
...
async function showAccount(){
 ...
 if(!cloud.user){modal(authScreen());return;}
...
async function refreshCloud(){
 if(cloud.guest||!cloud.ready||cloud.busy||cloud.running||cloud.pending||document.hidden||document.querySelector('#modal').open||document.activeElement?.matches('input,select,textarea'))return;
